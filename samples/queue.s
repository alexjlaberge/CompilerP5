  ...
  _QueueItem.Init:
	# BeginFunc 0
	  ...
	  lw $t1, 12($fp)	# fill next to $t1 from $fp+12
  ...
  _L0:
	...
	# _tmp39 = ACall _tmp38
	  ...
	  jalr $t0            	# jump to function
	  move $t2, $v0		# copy function return value from $v0
	  lw $t4, -8($fp)	# fill val to $t4 from $fp-8
	  lw $t3, -44($fp)	# fill temp to $t3 from $fp-44
	# PopParams 4
	  add $sp, $sp, 4	# pop params off stack
	# _tmp40 = *(temp)
	...
	# _tmp50 = ACall _tmp49
	  ...
	  move $t1, $v0		# copy function return value from $v0
	  lw $t4, -8($fp)	# fill val to $t4 from $fp-8
	  lw $t2, -116($fp)	# fill _tmp47 to $t2 from $fp-116
	# PopParams 4
	  add $sp, $sp, 4	# pop params off stack
	# _tmp51 = *(_tmp50)
	  lw $t0, 0($t1) 	# load with offset
  ...
  main:
	...
	# ACall _tmp57
	  ...
	  jalr $t0            	# jump to function
	  lw $t2, -8($fp)	# fill q to $t2 from $fp-8
	# PopParams 4
	  add $sp, $sp, 4	# pop params off stack
	# _tmp58 = 0
	  li $t0, 0		# load constant value 0 into $t0
